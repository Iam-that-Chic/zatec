                                ...
                                <div class="col-md-4">
                                    <div class="card">
                                        ...
                                        @if (in_array($album->artist, $favalbums) && in_array($album->name, array_keys($favalbums)))
                                            ...
                                            <span class="fa fa-heart liked-icon" title="LIKED"
                                                onclick="unFavAlbum('{{ $album->name }}', '{{ $album->artist }}')"></span>
                                        @else
                                            <span class="fa fa-heart like-icon"
                                                onclick="myFavAlbum('{{ $album->name }}', '{{ $album->artist }}', '{{ $album->mbid }}')"
                                                title="LIKE ME"></span>
                                        @endif
                                        <div class="card-body">
                                        ...
                                        <div class="card-footer">
                                            <a class="btn btn-info"
                                                href="{{ route('showalbum', ['artist' => $album->artist, 'album' => $album->name]) }}">
                                                VIEW MORE</a>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="row no-gutters">
                                            <div class="col-md-4">
                                                @foreach ($artist->image as $image)
                                                    @if ($loop->last)
                                                        <img src="{{ $image->{'#text'} }}" class="card-img"
                                                            alt="..." />
                                                    @endif
                                                @endforeach
                                            </div>
                                            <div class="col-md-8">
                                                <div class="card-body">
                                                    ...
                                                    @if (in_array($artist->name, $favartists))
                                                        ...
                                                        <span class="fa fa-heart liked-icon"
                                                            onclick="unFavArtist('{{ $artist->name }}'"
                                                            title="LIKED"></span>
                                                    @else
                                                        <span class="fa fa-heart like-icon"
                                                            onclick="myFavArtist('{{ $artist->name }}', '{{ $artist->mbid }} ')"
                                                            title="LIKE ME"></span>
                                                    @endif
                                                </div>
                                            </div>
